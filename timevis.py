...
parser.add_argument("--filename", type=str,
                    help='CSV File that timevis will run on.')
parser.add_argument("--center_radius_mode", type=str,
                    help='[y/n] Should timevis run in center radius mode, or normal mode')
parser.add_argument('--force', type=str,
                    help='[y/n] If true, the preprocessor will overwrite previously preprocessed files, otherwise. If false, the preprocessor will not run, and will immediately handoff to the plotter')
parser.add_argument('--install', type=str,
                    help='[y/n] If true, all packages in requirements.txt will be installed, if necessary')
parser.add_argument('--python3', type=str,
                    help='[y/n] If true, python3 will be used by default', default='y')
parser.add_argument('--pip3', type=str,
                    help='[y/n] If true, pip3 will be used by default', default='y')
parser.add_argument('--userpip', type=str,
                    help='[y/n] If true, the -U flag will be added to pip', default='n')
parser.add_argument('--average', type=str,
                    help='[y/n] If true, the preprocess mode will be set to average', default='n')
...
if args.install == 'y' or args.install == 'Y':
    if (filecmp.cmp('requirements.txt', 'installed_packages.txt')):
        ...
        if args.userpip == 'y' or args.userpip == 'Y':
            ...
            subprocess.run(['cp', 'requirements.txt',
...
if not force:
    try:
        ...
        print("+ python plot.py")
        print(
            "open this link in your browser: file://%s/templates/index.html" % os.getcwd())
        server.run(base_data_path)
    ...
    if args.average:
        subprocess.run([python, './src/preprocess.py',
                        '--filename=%s' % args.filename, '--averagemode="y"'])
    else:
        subprocess.run([python, './src/preprocess.py',
                        '--filename=%s' % args.filename, '--averagemode="n"'])
