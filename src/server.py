...
def get_anomalous_points_for_chunk(chunk, dataset):
    ...
    if chunk == 0:
        ...
        data_range = ((chunk * G_MAX_VALUE) -
                      G_MAX_VALUE, (chunk * G_MAX_VALUE))
...
def read_normalized(datadir, level_tmp, low_scaled, high_scaled):
    tmp = []
    with open("./data/%s/level_%02d.csv" % (datadir, level_tmp), 'r') as r:
        reader = csv.reader(r, delimiter=',')
        for count, i in enumerate(reader):
            if count >= low_scaled and count <= high_scaled:
                tmp.append(i)
            elif count > high_scaled:
                break
    return tmp
...
def query_range(l, h):
    global datadir
    low, high = l, h
    progressive_decline = level_zero_size // (
        10 if level_zero_size == 10_000_000 else 100)
   ...
   # necessary data should be encapsulated from [2 million / (level+1), 3 million/(level+1)]
    ...
    raw_level = int((5*progressive_decline) // int(high - low))
    # TODO:  refactor
    ...
    if level_tmp == 0:
        # tmp = read_avg(datadir, level_tmp, low_scaled, high_scaled+low_scaled)
        tmp = read_normalized(datadir, level_tmp, low_scaled, high_scaled)
    else:
...
def get_range():
    ...
    if level == 0:
        tmp = []
        for i in range(result[-2], result[-1]):
            tmp.append(i)
        data = [tmp[:len(result[0])], result[0]]
        return json.dumps({
            "data": data,
            "level": result[1]
        })
...
def get_all_data():
    ...
    lows = []
    highs = []
    ys = csv_read("./data/%s/level_%02d.csv" %
                  (plot_type, level))[:G_MAX_VALUE]
...
def get_all_data_for_chunk():
    ...
    ys = csv_read("./data/%s/level_%02d.csv" %
                  (plot_type, level))[data_min:data_max]
    tmp = []
...
def run(dd):
    global datadir
    global level_zero_size
    datadir = dd
    fname = f"./data/{datadir}/level_00.csv"
    try:
        ...
        if p.returncode != 0:
            raise IOError(err)
        level_zero_size = int(result.strip().split()[0])
    except Exception as e:
        print(e)
        level_zero_size = 10_000_000
